...
public:
  ...
  int fill_level, pf_origin_level, rob_signal, rob_index, producer, delta, depth, signature, confidence;
  ...
  uint64_t address, full_addr, instruction_pa, data_pa, data, instr_id, ip, event_cycle, cycle_enqueued;
...
public:
  const string NAME;
  const uint32_t SIZE;
  uint32_t cpu, head, tail, occupancy, last_read, last_fetch, last_scheduled, inorder_fetch[2], next_fetch[2], next_schedule;
  uint64_t event_cycle, fetch_event_cycle, schedule_event_cycle, execute_event_cycle, lsq_event_cycle, retire_event_cycle;

    std::vector<std::vector<ooo_model_instr*>> entry;
