...
void Options::init(CLI::App& app)
{
  ...
  app.add_option("--execute-width", execute_width, "The width of the execution unit");
  ...
  app.add_option("--mispredict-penalty", mispredict_penalty, "The penalty for a branch misprediction");
  ...
  app.add_option("--schedule-latency", schedule_latency, "The latency of the scheduler");
  app.add_option("--execute-latency", execute_latency, "The latency of the execution unit");
...
void Options::update(environment& env)
{
  for (O3_CPU& cpu : env.cpu_view()) {
    ...
    if (mispredict_penalty)
      cpu.BRANCH_MISPREDICT_PENALTY = mispredict_penalty;
    if (scheduler_size)
    ...
    if (schedule_latency)
      cpu.SCHEDULING_LATENCY = schedule_latency;
    if (execute_latency)
      cpu.EXEC_LATENCY = execute_latency;
