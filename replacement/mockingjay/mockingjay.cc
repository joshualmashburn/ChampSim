...
#if 0
#define LLC_WAY NUM_WAY
#define LLC_SET NUM_SET
#endif
...
//  uint32_t CACHE::find_victim(uint32_t triggering_cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t ip, uint64_t full_addr, uint32_t
//  type)
uint32_t CACHE::find_victim(uint32_t triggering_cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t pc, uint64_t full_addr, uint32_t type)
{
  ...
  uint64_t pc_signature = get_pc_signature(pc, false, type == (uint32_t)access_type::PREFETCH, triggering_cpu);
  if (type != (uint32_t)access_type::WRITE && rdp.count(pc_signature) && (rdp[pc_signature] > MAX_RD || rdp[pc_signature] / GRANULARITY > max_etr)) {
    return LLC_WAY;
  }
...
// update_replacement_state(uint32_t triggering_cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t
// hit)
// void CACHE::llc_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t pc, uint64_t victim_addr, uint32_t type,
// uint8_t hit)
                                     ...
                                     uint8_t hit)
{
  ...
  pc = get_pc_signature(pc, hit, type == (uint32_t)access_type::PREFETCH, triggering_cpu);
  ...
  if (is_sampled_set(set)) {
    ...
    if (sampled_cache_way > -1) {
      ...
      if (sample <= INF_RD) {
        if (type == (uint32_t)access_type::PREFETCH) {
