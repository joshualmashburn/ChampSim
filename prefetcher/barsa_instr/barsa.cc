...
cache_block shadow_cache[MAX_NUM_SET][MAX_NUM_WAY];
...
void move_to_mru (cache_block *S, int set, int r) {
	...
	for (int i=0; i<MAX_NUM_WAY; i++) if (S[i].lruposition < position) S[i].lruposition++;
	S[r].lruposition = 0;
}
...
void init_cache (void) {
	for (int set=0; set<MAX_NUM_SET; set++) {
		cache_block *S = &shadow_cache[set][0];
		for (int way=0; way<MAX_NUM_WAY; way++) {
...
void access_cache (
	...
	int set = block_addr % MAX_NUM_SET;
	uint64_t tag = (block_addr / MAX_NUM_SET) & ((1ull<<CACHE_PARTIAL_TAG_BITS)-1);
	...
	if (real_victim) {
		...
		int rvset = (real_victim / BLOCK_SIZE) % MAX_NUM_SET;
		assert (rvset == set);

		// go through each way looking for this victim

		for (int i=0; i<MAX_NUM_WAY; i++) {
			// figure out the (partial) address of this block

			uint64_t my_addr = ((S[i].tag * MAX_NUM_SET) | set) * BLOCK_SIZE;

			// prepare a mask to allow comparing the shorter addresses

			uint64_t mask = ((1ull<<CACHE_PARTIAL_TAG_BITS) * MAX_NUM_SET * BLOCK_SIZE) - 1;
	...
	int r;
	for (r=0; r<MAX_NUM_WAY; r++) if (S[r].valid == false) break;

	// no invalid block? get the LRU block

	if (r == MAX_NUM_WAY) {
		r = 0;
		for (int i=0; i<MAX_NUM_WAY; i++) if (S[i].lruposition == MAX_NUM_WAY-1) r = i;
		assert (r != MAX_NUM_WAY);
	}
...
void CACHE::prefetcher_cycle_operate() {
	...
	for (int i=0; i<dequeue_per_cycle; i++) {
		...
		if (size(internal_PQ) < PQ_SIZE) {
			...
			} else if ((size(internal_PQ) == 0) && would_be_nice_queue.size()) {
			...
			prefetch_line (p.pf_addr, true, 0);
