...
namespace knob
{
vector<int32_t> leh_featurewise_active_features = {0, 10};
vector<int32_t> leh_featurewise_num_tilings = {3, 3};
vector<int32_t> leh_featurewise_num_tiles = {128, 128};
vector<int32_t> leh_featurewise_hash_types = {2, 2};
vector<int32_t> leh_featurewise_enable_tiling_offset = {1, 1};
float leh_featurewise_max_q_thresh = 0.50;
bool leh_featurewise_enable_action_fallback = true;
vector<float> leh_featurewise_feature_weights = {1.00, 1.00};
bool leh_featurewise_enable_dynamic_weight = false;
float leh_featurewise_weight_gradient = 0.001; /* hyperparameter */
bool leh_featurewise_disable_adjust_weight_all_features_align = true;
bool leh_featurewise_selective_update = false;
uint32_t leh_featurewise_pooling_type = 2;
bool leh_featurewise_enable_dyn_action_fallback = true;
uint32_t leh_featurewise_bw_acc_check_level = 1;
uint32_t leh_featurewise_acc_thresh = 2;
bool leh_featurewise_enable_trace = false;
std::string leh_featurewise_trace_file_name;
bool leh_featurewise_enable_score_plot;
vector<int32_t> leh_featurewise_plot_actions;
std::string leh_featurewise_plot_file_name;
bool leh_featurewise_remove_plot_script;
} // namespace knob

void LearningEngineFeaturewise::init_knobs()
{
  assert(knob::leh_featurewise_active_features.size() == knob::leh_featurewise_num_tilings.size());
  assert(knob::leh_featurewise_active_features.size() == knob::leh_featurewise_num_tiles.size());
  assert(knob::leh_featurewise_active_features.size() == knob::leh_featurewise_enable_tiling_offset.size());
  assert(knob::leh_featurewise_active_features.size() == knob::leh_featurewise_feature_weights.size());
  if (knob::leh_featurewise_enable_dyn_action_fallback) {
    assert(knob::leh_featurewise_enable_action_fallback);
  }
}

void LearningEngineFeaturewise::init_stats() {}

LearningEngineFeaturewise::LearningEngineFeaturewise(Scooby* parent, float alpha, float gamma, float epsilon, uint32_t actions, uint64_t seed,
    ...
    : LearningEngineBase(parent, alpha, gamma, epsilon, actions, 0 /*dummy state value*/, seed, policy, type)
{
  ...
  for (uint32_t index = 0; index < knob::leh_featurewise_active_features.size(); ++index) {
    assert(knob::leh_featurewise_active_features[index] < NumFeatureTypes);
    m_feature_knowledges[knob::leh_featurewise_active_features[index]] =
        new FeatureKnowledge((FeatureType)knob::leh_featurewise_active_features[index], alpha, gamma, actions, knob::leh_featurewise_feature_weights[index],
                             knob::leh_featurewise_weight_gradient, knob::leh_featurewise_num_tilings[index], knob::leh_featurewise_num_tiles[index], zero_init,
                             knob::leh_featurewise_hash_types[index], knob::leh_featurewise_enable_tiling_offset[index]);
    assert(m_feature_knowledges[knob::leh_featurewise_active_features[index]]);
  }

  m_max_q_value = (float)1 / (1 - gamma) * std::accumulate(knob::leh_featurewise_num_tilings.begin(), knob::leh_featurewise_num_tilings.end(), 0);
                                      ...
                                      RewardType reward_type)
{
  ...
  if (m_type == LearningType::SARSA && m_policy == Policy::EGreedy) {
    for (uint32_t index = 0; index < NumFeatureTypes; ++index) {
      if (m_feature_knowledges[index]) {
        if (!knob::leh_featurewise_selective_update || consensus_vec[index]) {
          m_feature_knowledges[index]->updateQ(state1, action1, reward, state2, action2);
        } else if (knob::leh_featurewise_selective_update && !consensus_vec[index]) {
          stats.learn.su_skip[index]++;
        }
      }
    }

    if (knob::leh_featurewise_enable_dynamic_weight) {
...
uint32_t LearningEngineFeaturewise::getMaxAction(State* state, float& max_q, float& max_to_avg_q_ratio, vector<bool>& consensus_vec)
{
  ...
  if (max_q_value < knob::leh_featurewise_max_q_thresh * m_max_q_value) {
    max_to_avg_q_ratio = 0.0;
  }
  max_q = max_q_value;
...
float LearningEngineFeaturewise::consultQ(State* state, uint32_t action)
{
  ...
  for (uint32_t index = 0; index < NumFeatureTypes; ++index) {
    if (m_feature_knowledges[index]) {
      if (knob::leh_featurewise_pooling_type == 1) /* sum pooling */
      {
        q_value += m_feature_knowledges[index]->retrieveQ(state, action);
      } else if (knob::leh_featurewise_pooling_type == 2) /* max pooling */
      {
...
void LearningEngineFeaturewise::dump_stats()
{
  ...
  /* score plotting */
  if (knob::leh_featurewise_enable_trace && knob::leh_featurewise_enable_score_plot) {
    plot_scores();
  }
}
...
void LearningEngineFeaturewise::adjust_feature_weights(vector<bool> consensus_vec, RewardType reward_type)
{
  ...
  if (knob::leh_featurewise_disable_adjust_weight_all_features_align
      && (uint32_t)std::accumulate(consensus_vec.begin(), consensus_vec.end(), 0) == knob::leh_featurewise_active_features.size()) {
    return;
  }
...
bool LearningEngineFeaturewise::do_fallback(State* state)
{
  ...
  if (!knob::leh_featurewise_enable_dyn_action_fallback) {
    return knob::leh_featurewise_enable_action_fallback;
  }

  if (state->is_high_bw) {
    stats.action.dyn_fallback_saved_bw++;
    return false;
  } else if (state->bw_level >= knob::leh_featurewise_bw_acc_check_level && state->acc_level <= knob::leh_featurewise_acc_thresh) {
    stats.action.dyn_fallback_saved_bw_acc++;
    return false;
  }
...
void LearningEngineFeaturewise::plot_scores()
{
  ...
  fprintf(script, "set output '%s'\n", knob::leh_featurewise_plot_file_name.c_str());
  ...
  fprintf(script, "plot ");
  for (uint32_t index = 0; index < knob::leh_featurewise_plot_actions.size(); ++index) {
    if (index)
      fprintf(script, ", ");
    int action = scooby->getAction(knob::leh_featurewise_plot_actions[index]);
    stringstream ss;
    if (action > 0)
      ss << "+" << action;
    else
      ss << "-" << action;
    fprintf(script, "'%s' using 1:%u with lines ls %u title \"(%s)\"", knob::leh_featurewise_trace_file_name.c_str(),
            (knob::leh_featurewise_plot_actions[index] + 2), (index + 1), ss.str().c_str());
  }
  fprintf(script, "\n");
  fclose(script);
  ...
  if (knob::leh_featurewise_remove_plot_script) {
