...
namespace knob
{
bool leh_enable_trace;
uint32_t leh_trace_interval;
std::string leh_trace_file_name;
uint32_t leh_trace_state;
bool leh_enable_score_plot;
std::vector<int32_t> leh_plot_actions;
std::string leh_plot_file_name;
bool leh_enable_action_trace;
uint32_t leh_action_trace_interval;
std::string leh_action_trace_name;
bool leh_enable_action_plot;
} // namespace knob

LearningEngineBasic::LearningEngineBasic(Scooby* parent, float alpha, float gamma, float epsilon, uint32_t actions, uint32_t states, uint64_t seed,
                                         std::string policy, std::string type, bool zero_init, uint64_t early_exploration_window)
    : LearningEngineBase(parent, alpha, gamma, epsilon, actions, states, seed, policy, type)
{
  ...
  if (knob::leh_enable_trace) {
    trace_interval = 0;
    trace_timestamp = 0;
    trace = fopen(knob::leh_trace_file_name.c_str(), "w");
    assert(trace);
  }

  if (knob::leh_enable_action_trace) {
    action_trace_interval = 0;
    action_trace_timestamp = 0;
    action_trace = fopen(knob::leh_action_trace_name.c_str(), "w");
    assert(action_trace);
  }
...
LearningEngineBasic::~LearningEngineBasic()
{
  ...
  free(qtable);
  if (knob::leh_enable_trace && trace) {
    fclose(trace);
  }
}
...
uint32_t LearningEngineBasic::chooseAction(uint32_t state)
{
  ...
  if (m_type == LearningType::SARSA && m_policy == Policy::EGreedy) {
    if (m_action_counter < m_early_exploration_window || (*explore)(generator)) {
  ...
  if (knob::leh_enable_action_trace && action_trace_interval++ == knob::leh_action_trace_interval) {
...
void LearningEngineBasic::learn(uint32_t state1, uint32_t action1, int32_t reward, uint32_t state2, uint32_t action2)
{
  ...
  if (m_type == LearningType::SARSA && m_policy == Policy::EGreedy) {
    ...
    if (knob::leh_enable_trace && state1 == knob::leh_trace_state && trace_interval++ == knob::leh_trace_interval) {
...
void LearningEngineBasic::dump_stats()
{
  ...
  if (knob::leh_enable_trace && knob::leh_enable_score_plot) {
    plot_scores();
  }
}
...
void LearningEngineBasic::plot_scores()
{
  ...
  fprintf(script, "plot ");
  for (uint32_t index = 0; index < knob::leh_plot_actions.size(); ++index) {
    if (index)
      fprintf(script, ", ");
    fprintf(script, "'%s' using 1:%u with lines title \"action_index[%u]\"", knob::leh_trace_file_name.c_str(), (knob::leh_plot_actions[index] + 2),
            knob::leh_plot_actions[index]);
  }
  fprintf(script, "\n");
  fclose(script);
