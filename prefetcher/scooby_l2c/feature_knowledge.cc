...
FeatureKnowledge::FeatureKnowledge(FeatureType feature_type, float alpha, float gamma, uint32_t actions, float weight, float weight_gradient,
                                   uint32_t num_tilings, uint32_t num_tiles, bool zero_init, uint32_t hash_type, int32_t enable_tiling_offset)
    : m_feature_type(feature_type), m_alpha(alpha), m_gamma(gamma), m_actions(actions), m_weight(weight), m_weight_gradient(weight_gradient),
      m_hash_type(hash_type), m_num_tilings(num_tilings), m_num_tiles(num_tiles), m_enable_tiling_offset(enable_tiling_offset ? true : false)
{
...
void FeatureKnowledge::updateQ(State* state1, uint32_t action1, int32_t reward, State* state2, uint32_t action2)
{
  ...
  /* tracing Q-values */
  if (knob::le_featurewise_enable_trace && knob::le_featurewise_trace_feature_type == m_feature_type
      && !knob::le_featurewise_trace_feature.compare(get_feature_string(state1)) && trace_interval++ == knob::le_featurewise_trace_interval
