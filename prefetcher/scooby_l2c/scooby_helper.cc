...
namespace knob
{
uint32_t scoobyh_max_pcs = 5;
uint32_t scoobyh_max_offsets = 5;
uint32_t scoobyh_max_deltas = 5;
uint32_t scoobyh_state_type = 1;
uint32_t scoobyh_max_states = 1024;
uint32_t scoobyh_state_hash_type = 11;
bool scoobyh_enable_pt_address_compression;
uint32_t scoobyh_pt_address_hash_type;
uint32_t scoobyh_pt_address_hashed_bits;
bool scoobyh_access_debug = false;
uint64_t scoobyh_print_access_debug_pc;
uint32_t scoobyh_print_access_debug_pc_count;
uint32_t scoobyh_seed = 200;
uint32_t scoobyh_bloom_filter_size;
bool scoobyh_enable_dyn_degree_detector;
bool scoobyh_print_trace;
uint32_t scoobyh_action_tracker_size = 2;
} // namespace knob
...
{
  uint64_t value = 0;
  switch (knob::scoobyh_state_type) {
  case 1: /* Only PC */
  ...
  assert(hashed_value < knob::scoobyh_max_states);
...
{
  uint32_t value = 0;
  switch (knob::scoobyh_state_hash_type) {
  case 1: /* simple modulo */
    value = (uint32_t)(key % knob::scoobyh_max_states);
    break;

  case 2:
    value = HashZoo::jenkins((uint32_t)key);
    value = (uint32_t)(value % knob::scoobyh_max_states);
    break;
...
void Scooby_STEntry::update(uint64_t page, uint64_t pc, uint32_t offset, uint64_t address)
{
  ...
  /* insert PC */
  if (this->pcs.size() >= knob::scoobyh_max_pcs) {
    this->pcs.pop_front();
  }
  ...
  if (!this->offsets.empty()) {
    ...
    if (this->deltas.size() >= knob::scoobyh_max_deltas) {
  ...
  /* insert offset */
  if (this->offsets.size() >= knob::scoobyh_max_offsets) {
...
void Scooby_STEntry::insert_action_tracker(int32_t pref_offset)
{
  ...
  if (it != action_tracker.end()) {
    ...
    if (action_tracker.size() >= knob::scoobyh_action_tracker_size) {
...
void ScoobyRecorder::record_trigger_access(uint64_t page, uint64_t pc, uint32_t offset) { unique_trigger_pcs.insert(pc); }
...
{
  bool to_print = true;
  if (knob::scoobyh_print_access_debug_pc) {
    if (stentry->unique_pcs.find(knob::scoobyh_print_access_debug_pc) == stentry->unique_pcs.end()) {
      to_print = false;
    } else {
      debug_print_count++;
      if (debug_print_count > knob::scoobyh_print_access_debug_pc_count) {
        to_print = false;
      }
    }
  }
  ...
  fprintf(stdout, "[ACCESS] %16lx|%16lx|%2u|%2u|%64s|%64s|%2u|%2u|%2u|%2u|%2u|", stentry->page, trigger_pc, trigger_offset, unique_pc_count,
          BitmapHelper::to_string(stentry->bmp_real).c_str(), BitmapHelper::to_string(stentry->bmp_pred).c_str(),
          BitmapHelper::count_bits_set(stentry->bmp_real), BitmapHelper::count_bits_set(stentry->bmp_pred),
