...
uint32_t dspatch_num_cachelines_in_region = 32;
...
void DSPatch::init_stats() { bzero(&stats, sizeof(stats)); }
...
void DSPatch::update_bw(uint64_t bw)
{
...
uint32_t DSPatch::get_hash(uint32_t key)
{
  ...
  case 1:
    return key;
  case 2:
    return HashZoo::jenkins(key);
  case 3:
    return HashZoo::knuth(key);
  case 4:
    return HashZoo::murmur3(key);
  case 5:
    return HashZoo::jenkins32(key);
  case 6:
    return HashZoo::hash32shift(key);
  case 7:
    return HashZoo::hash32shiftmult(key);
  case 8:
    return HashZoo::hash64shift(key);
  case 9:
    return HashZoo::hash5shift(key);
  case 10:
    return HashZoo::hash7shift(key);
  case 11:
    return HashZoo::Wang6shift(key);
  case 12:
    return HashZoo::Wang5shift(key);
  case 13:
    return HashZoo::Wang4shift(key);
  case 14:
    return HashZoo::Wang3shift(key);
  default:
    assert(false);
  }
}
...
void DSPatch::add_to_spt(DSPatch_PBEntry* pbentry)
{
  ...
  sptentry->bmp_acc = BitmapHelper::bitwise_and(
      bmp_real, BitmapHelper::decompress(sptentry->bmp_cov, knob::dspatch_compression_granularity, knob::dspatch_num_cachelines_in_region));
...
void DSPatch::dump_stats()
{
  ...
  cout << "dspatch.bw.called " << stats.bw.called << endl << "dspatch.bw.bw_histogram ";
...
void CACHE::prefetcher_initialize() { prefetcher = new DSPatch(); }
