...
namespace knob
{
	...
	uint32_t sms_pht_size = 2048;
	uint32_t sms_pht_assoc = 16;
	uint32_t sms_pref_degree = 4;
	uint32_t sms_region_size = 4096;
	uint32_t sms_region_size_log = 12;
	bool sms_enable_pref_buffer = true;
	uint32_t sms_pref_buffer_size = 256;
} // namespace knob

void SMSPrefetcher::init_knobs() { pht_sets = knob::sms_pht_size / knob::sms_pht_assoc; }

void SMSPrefetcher::init_stats() { memset(&stats, 0, sizeof(stats)); }
...
deque<FTEntry*>::iterator SMSPrefetcher::search_victim_filter_table() { return filter_table.begin(); }
...
void CACHE::prefetcher_initialize() { prefetcher = new SMSPrefetcher(); }
