...
uint32_t bingo_pht_size = 4096;
uint32_t bingo_pht_ways = 16;
uint32_t bingo_pf_streamer_size = 128;
uint32_t bingo_debug_level = 0;
float bingo_l1d_thresh = 1.01;
float bingo_l2c_thresh = 0.80;
float bingo_llc_thresh = 0.05;
string bingo_pc_address_fill_level = "L2";
} // namespace knob

void Bingo::init_knobs() { assert((knob::bingo_region_size >> LOG2_BLOCK_SIZE) == knob::bingo_pattern_len); }

void Bingo::init_stats() {}

Bingo::Bingo(CACHE* cache)
    : parent(cache), pattern_len(knob::bingo_pattern_len), filter_table(knob::bingo_ft_size, knob::bingo_debug_level),
      accumulation_table(knob::bingo_at_size, knob::bingo_pattern_len, knob::bingo_debug_level),
      pht(knob::bingo_pht_size, knob::bingo_pattern_len, knob::bingo_min_addr_width, knob::bingo_max_addr_width, knob::bingo_pc_width, knob::bingo_debug_level,
          knob::bingo_pht_ways),
      pf_streamer(knob::bingo_pf_streamer_size, knob::bingo_pattern_len, knob::bingo_debug_level), debug_level(knob::bingo_debug_level)
{
  ...
  if (!knob::bingo_pc_address_fill_level.compare("L1"))
    pc_address_fill_level = cache->FILL_L1;
  else if (!knob::bingo_pc_address_fill_level.compare("L2"))
    pc_address_fill_level = cache->FILL_L2;
  else if (!knob::bingo_pc_address_fill_level.compare("LLC"))
    pc_address_fill_level = cache->FILL_LLC;
}
...
void Bingo::print_stats()
{
  ...
  double l1_pref_per_region = 1.0 * this->pref_level_cnt[parent->FILL_L1] / this->region_pref_cnt;
  double l2_pref_per_region = 1.0 * this->pref_level_cnt[parent->FILL_L2] / this->region_pref_cnt;
  double l3_pref_per_region = 1.0 * this->pref_level_cnt[parent->FILL_LLC] / this->region_pref_cnt;
...
{
  if (debug_level >= 2) {
    cerr << "CACHE::l1d_prefetcher_operate(addr=0x" << hex << addr << ", PC=0x" << pc << ", cache_hit=" << dec << (int)cache_hit << ", type=" << (int)type
         << ")" << dec << endl;
  }

  if (static_cast<access_type>(type) != access_type::LOAD)
    return;
...
void Bingo::dump_stats() { print_stats(); }

Bingo* prefetcher;

void CACHE::prefetcher_initialize() { prefetcher = new Bingo(this); }
...
{
  if (prefetcher)
    prefetcher->register_fill(addr, set, way, prefetch, evicted_addr);
  return metadata_in;
}
