...
namespace knob
{
int32_t ppfh_perc_threshold_hi = -5;
int32_t ppfh_perc_threshold_lo = -15;
} // namespace knob
...
void SIGNATURE_TABLE::read_and_update_sig(uint64_t page, uint32_t page_offset, uint32_t& last_sig, uint32_t& curr_sig, int32_t& delta)
{
  ...
  uint32_t set = spp_ppf::get_hash(page) % ST_SET, match = ST_WAY, partial_page = page & ST_TAG_MASK;
  uint8_t ST_hit = 0;
  int sig_delta = 0;
...
void PATTERN_TABLE::update_pattern(uint32_t last_sig, int curr_delta)
{
  ...
  uint32_t set = spp_ppf::get_hash(last_sig) % PT_SET, match = 0;
...
{
  // Update (sig, delta) correlation
  uint32_t set = spp_ppf::get_hash(curr_sig) % PT_SET, local_conf = 0, pf_conf = 0, max_conf = 0;
  ...
  if (c_sig[set]) {
    for (uint32_t way = 0; way < PT_WAY; way++) {
      ...
      bool do_pf = (perc_sum >= knob::ppfh_perc_threshold_lo) ? 1 : 0;
      bool fill_l2 = (perc_sum >= knob::ppfh_perc_threshold_hi) ? 1 : 0;
      ...
      if (pf_conf && do_pf && pf_q_tail < 100) {
        ...
        SPP_DP(cout << "[PT] State of Features: \nTrain addr: " << train_addr << "\tCurr IP: " << curr_ip << "\tIP_1: " << ghr->ip_1 << "\tIP_2: " << ghr->ip_2
                    << "\tIP_3: " << ghr->ip_3 << "\tDelta: " << train_delta + delta[set][way] << "\tLastSig: " << last_sig << "\tCurrSig: " << curr_sig
                    << "\tConf: " << pf_conf << "\tDepth: " << depth << "\tSUM: " << perc_sum << endl;);
...
{
  uint64_t cache_line = check_addr >> LOG2_BLOCK_SIZE, hash = spp_ppf::get_hash(cache_line);
  ...
  SPP_DP(cout << "[FILTER] check_addr: " << hex << check_addr << " check_cache_line: " << (check_addr >> LOG2_BLOCK_SIZE);
         cout << " request type: " << filter_request; cout << " hash: " << hash << dec << " quotient: " << quotient << " remainder: " << remainder << endl;);
  ...
  case SPP_PERC_REJECT: // To see what would have been the prediction given perceptron has rejected the PF
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      ...
      SPP_DP(cout << "[FILTER] " << __func__ << " PF rejected by perceptron. Set valid_reject for check_addr: " << hex << check_addr
                  << " cache_line: " << cache_line << dec;
             cout << " quotient: " << quotient << " remainder_tag: " << remainder_tag_reject[quotient_reject] << endl;
             cout << " More Recorded Metadata: Addr: " << hex << address_reject[quotient_reject] << dec << " PC: " << pc_reject[quotient_reject]
                  << " Delta: " << delta_reject[quotient_reject] << " Last Signature: " << last_signature_reject[quotient_reject]
                  << " Current Signature: " << cur_signature_reject[quotient_reject] << " Confidence: " << confidence_reject[quotient_reject] << endl;);
    }
    break;
  ...
  case SPP_L2C_PREFETCH:
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      ...
      SPP_DP(cout << "[FILTER] " << __func__ << " set valid for check_addr: " << hex << check_addr << " cache_line: " << cache_line << dec;
             ...
             cout << " More Recorded Metadata: Addr:" << hex << address[quotient] << dec << " PC: " << pc[quotient] << " Delta: " << delta[quotient]
  ...
  case L2C_DEMAND:
    if ((remainder_tag[quotient] == remainder) && (useful[quotient] == 0)) {
      ...
      SPP_DP(cout << "[FILTER] " << __func__ << " set useful for check_addr: " << hex << check_addr << " cache_line: " << cache_line << dec;
             ...
             if (valid[quotient]) cout << " Calling Perceptron Update (INC) as L2C_DEMAND was useful" << endl;);
    ...
    if (!(valid[quotient] && remainder_tag[quotient] == remainder)) {
      ...
      if (valid_reject[quotient_reject] && remainder_tag_reject[quotient_reject] == remainder_reject) {
        SPP_DP(cout << "[FILTER] " << __func__ << " not doing anything for check_addr: " << hex << check_addr << " cache_line: " << cache_line << dec;
  ...
  case L2C_EVICT:
    ...
    if (valid[quotient] && !useful[quotient]) {
      ...
      SPP_DP(cout << "[FILTER] " << __func__ << " eviction for check_addr: " << hex << check_addr << " cache_line: " << cache_line << dec;
             cout << " quotient: " << quotient << " valid: " << valid[quotient] << " useful: " << useful[quotient] << endl;
             cout << " Calling Perceptron Update (DEC) as L2C_DEMAND was not useful" << endl; cout << " Reseting valid_reject" << endl;);
                                ...
                                uint32_t curr_sig, uint32_t confidence, uint32_t depth, uint64_t perc_set[PERC_FEATURES])
{
  ...
  for (int i = 0; i < PERC_FEATURES; i++) {
    ...
    SPP_DP(cout << "  Perceptron Set Index#: " << i << " = " << perc_set[i];);
  }
  SPP_DP(cout << endl;);
}
                                 ...
                                 uint32_t curr_sig, uint32_t confidence, uint32_t depth)
{
  SPP_DP(int sig_delta = (cur_delta < 0) ? (((-1) * cur_delta) + (1 << (SIG_DELTA_BIT - 1))) : cur_delta;
         ...
         cout << " Cur Delta: " << cur_delta << " Sign Delta: " << sig_delta << " Confidence: " << confidence << endl; cout << " ";);
  ...
  SPP_DP(cout << " Sum of perceptrons: " << sum
              << " Prediction made: " << ((sum >= knob::ppfh_perc_threshold_lo) ? ((sum >= knob::ppfh_perc_threshold_hi) ? FILL_L2 : FILL_LLC) : 0) << endl;);
  // Return the sum
  return sum;
}
                             ...
                             uint32_t curr_sig, uint32_t confidence, uint32_t depth, bool direction, int32_t perc_sum)
{
  ...
  if (!direction) { // direction = 1 means the sum was in the correct direction, 0 means it was in the wrong direction
    // Prediction wrong
    for (int i = 0; i < PERC_FEATURES; i++) {
      if (sum >= knob::ppfh_perc_threshold_hi) {
    ...
    SPP_DP(int differential = (sum >= knob::ppfh_perc_threshold_hi) ? -1 : 1; cout << " Direction is: " << direction << " and sum is:" << sum;
  ...
  if (direction && sum > NEG_UPDT_THRESHOLD && sum < POS_UPDT_THRESHOLD) {
    ...
    SPP_DP(int differential = 0; if (sum >= knob::ppfh_perc_threshold_hi) differential = 1; if (sum < knob::ppfh_perc_threshold_hi) differential = -1;
           cout << " Direction is: " << direction << " and sum is:" << sum; cout << " Overall Differential: " << differential << endl;);
  }
}