...
SPP_PPF_dev::SPP_PPF_dev(CACHE* cache) : m_parent_cache(cache)
{
  init_knobs();
  init_stats();
  print_config();
...
void SPP_PPF_dev::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit, uint8_t type, std::vector<uint64_t>& pref_addr)
{
  ...
  do {
    ...
    PT.read_pattern(curr_sig, delta_q, confidence_q, perc_sum_q, lookahead_way, lookahead_conf, pf_q_tail, depth, addr, base_addr, train_addr, curr_ip,
                    train_delta, last_sig, m_parent_cache->get_pq_occupancy().back(), m_parent_cache->get_pq_size().back(), m_parent_cache->get_mshr_occupancy(), m_parent_cache->get_mshr_size());
    ...
    for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
      ...
      if ((addr & ~(PAGE_SIZE - 1)) == (pf_addr & ~(PAGE_SIZE - 1))) { // Prefetch request is in the same physical page
        ...
        if (num_pf < ceil(((m_parent_cache->get_pq_size().back()) / distinct_pages))) {
          if (FILTER.check(pf_addr, train_addr, curr_ip, fill_level, train_delta + delta_q[i], last_sig, curr_sig, confidence_q[i], perc_sum, (depth - 1))) {
            ...
            m_parent_cache->prefetch_line(pf_addr, ((fill_level == SPP_L2C_PREFETCH) ? m_parent_cache->FILL_L2 : m_parent_cache->FILL_LLC), 5);
            num_pf++;
...
SPP_PPF_dev* prefetcher;

void CACHE::prefetcher_initialize() { prefetcher = new SPP_PPF_dev(this); }
