...
uint64_t l1i_get_bere_hist_table(uint64_t line_addr, uint64_t latency, uint32_t skip = 0)
{
  ...
  for (uint32_t count = 0, i = first; count < L1I_HIST_TABLE_ENTRIES; count++, i = (i + L1I_HIST_TABLE_MASK) % L1I_HIST_TABLE_ENTRIES) {
    ...
    if (req_time == 0 && l1i_hist_table[l1i_cpu_id][i].tag == tag && time_i + latency >= l1i_current_cycle) { // Its me (miss or late prefetcher)
      req_time = time_i;
...
#define L1I_TIMING_CACHE_TAG_BITS (L1I_TIMING_MSHR_TAG_BITS - MAX_NUM_SET_BITS)
...
uint64_t l1i_find_timing_cache_entry(uint64_t line_addr)
{
  ...
  for (uint32_t j = 0; j < MAX_NUM_WAY; j++) {
    if (l1i_timing_cache_table[l1i_cpu_id][i][j].tag == ((line_addr >> MAX_NUM_SET_BITS) & L1I_TIMING_CACHE_TAG_MASK)
...
void l1i_move_timing_entry(uint64_t line_addr)
{
  ...
  if (index_mshr == L1I_TIMING_MSHR_SIZE) {
    ...
    l1i_timing_cache_table[l1i_cpu_id][set][index_cache].tag = (line_addr >> MAX_NUM_SET_BITS) & L1I_TIMING_CACHE_TAG_MASK;
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t v_addr, uint64_t ip, uint8_t cache_hit, bool prefetch_hit, uint8_t type, uint32_t metadata_in)
{
  ...
  // Do prefetches
  while (size(internal_PQ) < PQ_SIZE && !l1i_empty_xpq()) {
...
void CACHE::prefetcher_final_stats() { cout << "CPU " << cpu << " L1I EPI prefetcher final stats" << endl; }
