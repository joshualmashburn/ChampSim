...
bool sandbox_enable_stream_detect = true;
uint32_t sandbox_stream_detect_length = 3;
uint32_t sandbox_num_access_in_phase = 256;
uint32_t sandbox_num_cycle_offsets = 4;
uint32_t sandbox_bloom_filter_size = 2048;
uint32_t sandbox_seed = 300;
} // namespace knob

void SandboxPrefetcher::init_knobs() { pref_degree = knob::sandbox_pref_degree; /* minimum prefetch degree is 4 */ }

void SandboxPrefetcher::init_stats() { memset(&stats, 0, sizeof(stats)); }
...
bool sort_function(Score* score1, Score* score2) { return (abs(score1->offset) < abs(score2->offset)); }
...
void SandboxPrefetcher::filter_add(uint64_t address) { bf->add(address); }

bool SandboxPrefetcher::filter_lookup(uint64_t address) { return bf->lookup(address); }
...
SandboxPrefetcher* prefetcher;

void CACHE::prefetcher_initialize() { prefetcher = new SandboxPrefetcher(); }
