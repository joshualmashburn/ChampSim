...
//                               Total 1008 bits
    ...
    bool update_training_stream_and_prefetch(uint64_t line_address, CACHE* pCACHE) {
        for (size_t i = 0; i < TrainingTableSize; ++i) {
            ...
            if (stream.count >= TrainingThreshold) {
                prefetch_initial_stream(line_address, stream, pCACHE);
    ...
    bool update_monitoring_stream_and_prefetch(uint64_t line_address, CACHE* pCACHE) {
        for (size_t i = 0; i < MonitoringTableSize; ++i) {
            ...
            for (size_t j = 0; j < Degree; ++j) {
                ...
                pCACHE->prefetch_line(pf_addr, true, 0);
    ...
    void prefetch_initial_stream(uint64_t line_address, const TrainingStreamEntry& stream, CACHE* pCACHE) {
...
public:
    void cache_operate(uint64_t address, uint8_t cache_hit, uint8_t prefetch_hit, CACHE* pCACHE) {
        const bool virtual_miss = cache_hit == 0 || prefetch_hit == 1;
        const uint64_t line_address = address >> LOG2_BLOCK_SIZE;

        const bool already_monitored_stream = update_monitoring_stream_and_prefetch(line_address, pCACHE);

        if (virtual_miss) {
            const bool already_training_stream = update_training_stream_and_prefetch(line_address, pCACHE);
...
public:
    D_JOLT_PREFETCHER(CACHE* pCACHE);
...
D_JOLT_PREFETCHER::D_JOLT_PREFETCHER(CACHE* cache) : pCACHE(cache) {
    ...
    if (table.contains(sig)) {
        for (const auto& v : table[sig].getValidEntries()) {
            for (const auto& address : v.getAddresses()) {
                ...
                const int success = pCACHE->prefetch_line(pf_addr, true, 0);
            }
        }
    }
}
...
void D_JOLT_PREFETCHER::cache_operate(uint64_t addr, uint8_t cache_hit, uint8_t prefetch_hit) {
    ...
    const bool virtual_miss = cache_hit == 0 || prefetch_hit == 1;
    stream_prefetcher.cache_operate(addr, cache_hit, prefetch_hit, pCACHE);
...
void CACHE::prefetcher_resize_cache(uint32_t set, uint32_t way) { prefetcher_initialize(); }