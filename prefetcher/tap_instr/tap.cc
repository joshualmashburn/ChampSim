...
struct shadow_cache_filter
{
    ...
    using cache_t = std::array<std::array<shadow_cache_filter_node_t, MAX_NUM_WAY>, MAX_NUM_SET>;
    const std::size_t nbits;
    const unsigned long long bitmask;
    cache_t shadow_cache;

    shadow_cache_filter(std::size_t nbits) : nbits(nbits), bitmask(((1<<nbits)-1) << _nbits(MAX_NUM_SET)) {}
...
{
    cache_t::value_type &set = shadow_cache.at(block_addr % MAX_NUM_SET);
    const addr_t tag = hashint(block_addr >> _nbits(MAX_NUM_SET)) & bitmask;
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t v_addr, uint64_t ip, uint8_t cache_hit, bool prefetch_hit, uint8_t type, uint32_t metadata_in)
{
    ...
    for (auto it = path.begin(); it != path.end(); ++it)
    {
        ...
        if (!filterhit)
        {
          bool space_left = prefetch_line(*it<<LOG2_BLOCK_SIZE, true, 0);
            if (space_left)
            {
