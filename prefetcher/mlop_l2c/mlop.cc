  ...
  if (this->update_cnt == NUM_UPDATES) {
    ...
    for (int d = PF_DEGREE - 1; d >= 0; d -= 1) {
      ...
      if (max_scores[d] >= (int)L2C_THRESH)
        fill_level = parent->FILL_L2;
      else if (max_scores[d] >= (int)LLC_THRESH)
        fill_level = parent->FILL_LLC;
      else
        continue;
    ...
    if (this->debug_level >= 1) {
      for (uint32_t d = 0; d < PF_DEGREE; d += 1) {
        ...
        if (this->pf_level[d] == 0)
          cout << "No";
        if (this->pf_level[d] == parent->FILL_L1)
          cout << "L1";
        if (this->pf_level[d] == parent->FILL_L2)
          cout << "L2";
        if (this->pf_level[d] == parent->FILL_LLC)
          cout << "L3";
...
void MLOP::mark(uint64_t block_number, MLOP_State state, int fill_level) { this->access_map_table->set_state(block_number, state, fill_level); }
...
void MLOP::dump_stats() { print_stats(); }
...
void CACHE::prefetcher_initialize() { prefetcher = new MLOP(this); }
