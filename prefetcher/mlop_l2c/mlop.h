...
public:
      ...
      : Super(size, num_ways, debug_level), blocks_in_zone(blocks_in_zone), queue_size(queue_size)
  {
    if (this->debug_level >= 1)
      cout << "AccessMapTable::AccessMapTable(size=" << size << ", blocks_in_zone=" << blocks_in_zone << ", queue_size=" << queue_size
  ...
  void set_state(uint64_t block_number, MLOP_State new_state, int new_fill_level = 0)
  {
    if (this->debug_level >= 2)
      cout << "AccessMapTable::set_state(block_number=0x" << hex << block_number << ", new_state=" << getStateChar(new_state)
    ...
    if (this->debug_level >= 2) {
      cout << "[AccessMapTable::set_state] zone_number=0x" << hex << zone_number << dec << ", zone_offset=" << setw(2) << zone_offset
           << ": state transition from " << getStateChar(old_state) << " to " << getStateChar(new_state) << endl;
      if (old_state != new_state || old_fill_level != new_fill_level) {
        cout << "[AccessMapTable::set_state] old_access_map=" << map_to_string(old_access_map, old_prefetch_map) << endl;
        cout << "[AccessMapTable::set_state] new_access_map=" << map_to_string(access_map, prefetch_map) << endl;
      }
    }
  }
