...
#define L1I_MERGE_BBSIZE_BITS 4
...
uint32_t l1i_find_bb_merge_hist_table(uint64_t line_addr) {
  ...
  for (uint32_t count = 0, i = (l1i_hist_table_head[l1i_cpu_id] + L1I_HIST_TABLE_MASK) % L1I_HIST_TABLE_ENTRIES; count < L1I_HIST_TABLE_ENTRIES; count++, i = (i + L1I_HIST_TABLE_MASK) % L1I_HIST_TABLE_ENTRIES) {
	...
	&& (tag - l1i_hist_table[l1i_cpu_id][i].tag) <= l1i_hist_table[l1i_cpu_id][i].bb_size) { // try: bb_size + 1
      if (l1i_hist_table[l1i_cpu_id][i].bb_size < L1I_MERGE_BBSIZE_MAX_VALUE) { // It can increase
	return tag - l1i_hist_table[l1i_cpu_id][i].tag;
      } else { // Basic block already in max size
	return 0;
      }
    }
  }
...
void CACHE::prefetcher_squash(uint64_t ip, uint64_t instr_id)
{
  ...
  // if (instr_id >= 2486067 && instr_id < 2486081) {
  //   std::cout << "Squash " << std::hex << line_addr << std::dec << " " << instr_id << std::endl;
  //   std::cout << "BB reg " << std::hex << l1i_last_basic_block << std::dec << " " << l1i_consecutive_count << " " << l1i_basic_block_merge_diff << std::endl;
  //   l1i_print_hist_table();
  // }

  l1i_squash_hist_table(instr_id, line_addr);
  uint32_t last = (l1i_hist_table_head[l1i_cpu_id] + L1I_HIST_TABLE_MASK) % L1I_HIST_TABLE_ENTRIES;
  if ((l1i_hist_table[l1i_cpu_id][last].tag <= line_addr
       && l1i_hist_table[l1i_cpu_id][last].tag + l1i_consecutive_count >= line_addr)
    ...
    assert(l1i_consecutive_count <= L1I_MERGE_BBSIZE_MAX_VALUE);
    l1i_current_head_instr_id = l1i_hist_table[l1i_cpu_id][last].instr_id;
  } else {
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool prefetch_hit, uint8_t type, uint32_t metadata_in)
{
  ...
  if (!wrong_path) assert(!l1i_stats_last_wrong_path);
  ...
  if (l1i_last_basic_block + l1i_consecutive_count == line_addr) { // Same
    return metadata_in;
	     ...
	     && l1i_consecutive_count < L1I_MERGE_BBSIZE_MAX_VALUE) { // No overflow
    l1i_consecutive_count++;
    if (wrong_path) l1i_contains_wrong_path = true;
    consecutive = true;
  }
  ...
  if (!consecutive) { // New basic block found
    ...
    if (l1i_consecutive_count) { // single blocks no need to merge and are not inserted in the entangled table
      if (l1i_basic_block_merge_diff > 0) {
   	l1i_add_bb_size_hist_table(l1i_last_basic_block, l1i_consecutive_count, l1i_contains_wrong_path);
      } else {
  ...
  if (!consecutive) { // New basic block found
    // Here we reset the basic block info! New basic block count starts now
    // Consider the whole merged block
    l1i_basic_block_merge_diff = l1i_find_bb_merge_hist_table(line_addr);
    assert(l1i_basic_block_merge_diff <= L1I_MERGE_BBSIZE_MAX_VALUE);
    l1i_consecutive_count = l1i_basic_block_merge_diff;
    l1i_last_basic_block = line_addr - l1i_basic_block_merge_diff;
    l1i_contains_wrong_path = false;
  }
  ...
  if (!consecutive && l1i_basic_block_merge_diff == 0) {
    if ((l1i_find_hist_entry(line_addr) == L1I_HIST_TABLE_ENTRIES)) {
      pos_hist = l1i_add_hist_table(line_addr, l1i_instr_id, format, num_entangled, wrong_path);
    } else {
