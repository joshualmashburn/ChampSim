...
class RECENT_PREFETCH_QUEUE
{
	...
	bool issue_prefetch(CACHE *cache, uint64_t addr, int lookahead_path) //lookahead_path == 0 (cache_operate); 1 (cycle_operate); 2(cycle_operate_other/temporal_table_path)
        {
			...
			else if(lookahead_path == 1)
			{
				...
				if(lookahead_path_selector.is_temporal_table_path_good())
					return true; //Don't Prefetch this path
                        ...
                        cache->prefetch_line(addr, true, 0);
                        return true;
                }
                return false;
        }
	...
	for(i = 0; i < prefetch_depth; i++)
		...
		//Case 1: Search in MJT2
                ...
                if(mjt2_present)
                {
                        ...
                        if (size(internal_PQ) < PQ_SIZE && pref_ip != 0 && recent_prefetch_queue.issue_prefetch(this,pref_ip, 0))
                        {
				pref_gen_this_cycle++;
                        }
                        processed_flag = 1;
		...
		if(!processed_flag)
                {
                        ...
                        {
				if (size(internal_PQ) < PQ_SIZE && recent_prefetch_queue.issue_prefetch(this, cur_ip, 0))
				{
					pref_gen_this_cycle++;
				}
                        }
...
void l1i_prefetcher_cycle_operate_other(CACHE* cache)
{
	...
	bool sjt_present, mjt1_present, mjt2_present;
		...
		//Case 1: Search in MJT2
                ...
                if(mjt2_present)
                {
                        ...
                        if (size(cache->internal_PQ) < cache->PQ_SIZE && pref_ip != 0 && recent_prefetch_queue.issue_prefetch(cache,pref_ip, 2))
                        {
				pref_gen_this_cycle++;
                        }
                        processed_flag = 1;
		...
		sjt_present = sjt.find(hash_ip);
                ...
                if(!processed_flag && sjt_present)
                {
                        ...
                        if((sjt_target >> (LOG2_BLOCK_SIZE))!= prev_line)
                        {
                                if (size(cache->internal_PQ) < cache->PQ_SIZE)
                                {
                                        pref_ip = sjt_target;
			
					if(recent_prefetch_queue.issue_prefetch(cache,pref_ip, 2))
						pref_gen_this_cycle++;
                ...
                if(!processed_flag)
                {
                        ...
                        if((cur_ip >> (LOG2_BLOCK_SIZE)) != prev_line)
                        {
				if (size(cache->internal_PQ) < cache->PQ_SIZE && recent_prefetch_queue.issue_prefetch(cache, cur_ip, 2)) 
				pref_gen_this_cycle++;
                        }
                        processed_flag = 1;
                }
...
/* We do this so that if the processor is continuously sending requests to the L1-I, we don't send prefetch requests that might get in the way of those demand requests. So if there are no processor requests for 2 cycles, we start the lookahead process in cycle_operate. */
	if((current_cycle - last_prefetch_cycle) < 2) 
		return;
	...
	bool sjt_present, mjt1_present, mjt2_present;
		...
		//Case 1: Search in MJT2
                ...
                if(mjt2_present)
                {
                        ...
                        if (size(internal_PQ) < PQ_SIZE && pref_ip != 0 && recent_prefetch_queue.issue_prefetch(this,pref_ip, 1))
                        {
				pref_gen_this_cycle++;
                        }
                        processed_flag = 1;
		...
		sjt_present = sjt.find(hash_ip);
                ...
                if(!processed_flag)
                {
                        ...
                        if((cur_ip >> (LOG2_BLOCK_SIZE)) != prev_line)
                        {
				if (size(internal_PQ) < PQ_SIZE && recent_prefetch_queue.issue_prefetch(this, cur_ip, 1)) 
				pref_gen_this_cycle++;
                        }
                        processed_flag = 1;
                }
